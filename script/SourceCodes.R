...
ProQuant <- function(raw,cond,rm){
  ...
  output$quan <- quan
  output$pep <- exprs(cdata)
  output$notice <- "Protein Quantification Completed."
  return(output)
}
...
Ratio_p <- function(quan, group, decoy, control_group=NA, is_t_test="t", pro_or_pep){
  ...
  #  pvals <- apply(quan_matrix,1,function(x)
  #   t.test(x[(rep_num*(con-1)+1):(rep_num*con)],
  #         x[(rep_num*(j-1)+1):(rep_num*j)],var.equal=TRUE)$p.value)
  #    p_chara[j] <- paste("pVal_",group_char[j],"/",control_group,sep="")
  #   ave_ratio <- cbind(ave_ratio,pvals)
  #}
  
  #Combine the calculated results with the original data frame
  if(pro_or_pep == 'pro'){
  ...
  #  ave_ratio2 <- ave_ratio
  #  ave_ratio2[,1:10] <- apply(as.matrix(ave_ratio2[,1:10]),c(1,2),function(x) round(x,digits=3))
  #  ave_ratio_dis <- ave_ratio2[,c(1,6,4,9)]
